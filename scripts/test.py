...
if __name__ == "__main__":
        ...
        if news_table:
            ...
            for row in stock_rows:
                ...
                if len(columns) == 2:
                    ...
                    # Display news in the Streamlit app
                    st.write(f"{stock_symbol} | {date} | {time} | {news_headline}")

        else:
            st.write("No news table found.")

    except Exception as e:
        st.write(f"Error fetching data from Finviz: {e}")
    ...
    news_df['compound'] = news_df['headline'].apply(lambda headline: vader.polarity_scores(headline)['compound'])
    news_df['date'] = pd.to_datetime(news_df.date).dt.date
    
    # Display news DataFrame with sentiment
